...
export async function loginService(req: {
  ...
  const ipAddress = (req.xForwardedFor as string) || (req.xRealIp as string) || "unknown";
  ...
  const token = crypto.randomBytes(32).toString("hex") + "." + req.jwtSecret;
  const tokenHash = crypto.createHash("sha256").update(token).digest("hex");
  const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);

  await db.exec`
    INSERT INTO sessions (user_id, token, expires_at, ip_address, user_agent)
    VALUES (${user.id}, ${tokenHash}, ${expiresAt}, ${ipAddress}, ${(req.userAgent as string) || ""})
  `;
